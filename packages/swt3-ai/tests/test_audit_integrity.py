...
from swt3_ai.witness import Witness, LOG_FORMAT_CODES
...
class TestWitnessAuditIntegrity:
    def test_mints_correct_procedure_and_factors(self):
        w = _make_witness()
        p = w.witness_audit_integrity(5000, True, "jsonl")
        assert p.procedure_id == "AI-AUDIT.1"
    ...
    def test_maps_all_type_codes(self):
        w = _make_witness()
        for code_name, code_value in LOG_FORMAT_CODES.items():
            p = w.witness_audit_integrity(5000, True, code_name)
            assert p.factor_c == float(code_value)

    def test_unknown_type_defaults(self):
        w = _make_witness()
        p = w.witness_audit_integrity(5000, True, "unknown_xyz")
    ...
    def test_context_at_clearing_level_1(self):
        ...
        assert "log_format" in p.ai_context
