...
from swt3_ai.witness import Witness, PROCESSING_TYPE_CODES
...
class TestWitnessDpia:
    def test_mints_correct_procedure_and_factors(self):
        w = _make_witness()
        p = w.witness_dpia(12, 10, "profiling")
        assert p.procedure_id == "AI-DPIA.1"
    ...
    def test_maps_all_type_codes(self):
        w = _make_witness()
        for code_name, code_value in PROCESSING_TYPE_CODES.items():
            p = w.witness_dpia(12, 10, code_name)
            assert p.factor_c == float(code_value)

    def test_unknown_type_defaults(self):
        w = _make_witness()
        p = w.witness_dpia(12, 10, "unknown_xyz")
    ...
    def test_context_at_clearing_level_1(self):
        ...
        assert "processing_type" in p.ai_context
