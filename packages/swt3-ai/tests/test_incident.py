...
from swt3_ai.witness import Witness, INCIDENT_TYPE_CODES
...
class TestWitnessIncident:
    def test_mints_correct_procedure_and_factors(self):
        w = _make_witness()
        p = w.witness_incident(3, True, "safety")
        assert p.procedure_id == "AI-INCIDENT.1"
    ...
    def test_maps_all_type_codes(self):
        w = _make_witness()
        for code_name, code_value in INCIDENT_TYPE_CODES.items():
            p = w.witness_incident(3, True, code_name)
            assert p.factor_c == float(code_value)

    def test_unknown_type_defaults(self):
        w = _make_witness()
        p = w.witness_incident(3, True, "unknown_xyz")
    ...
    def test_context_at_clearing_level_1(self):
        ...
        assert "incident_type" in p.ai_context
