...
from swt3_ai.witness import Witness, PMM_TYPE_CODES
...
class TestWitnessPostMarketMonitoring:
    def test_mints_correct_procedure_and_factors(self):
        w = _make_witness()
        p = w.witness_post_market_monitoring(200, 5, "performance")
        assert p.procedure_id == "AI-PMM.1"
    ...
    def test_maps_all_type_codes(self):
        w = _make_witness()
        for code_name, code_value in PMM_TYPE_CODES.items():
            p = w.witness_post_market_monitoring(200, 5, code_name)
            assert p.factor_c == float(code_value)

    def test_unknown_type_defaults(self):
        w = _make_witness()
        p = w.witness_post_market_monitoring(200, 5, "unknown_xyz")
    ...
    def test_context_at_clearing_level_1(self):
        ...
        assert "monitoring_type" in p.ai_context
