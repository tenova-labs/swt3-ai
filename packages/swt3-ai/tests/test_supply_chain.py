...
from swt3_ai.witness import Witness, SUPPLY_RISK_CODES
...
class TestWitnessSupplyChainRisk:
    def test_mints_correct_procedure_and_factors(self):
        w = _make_witness()
        p = w.witness_supply_chain_risk(10, 8, "low")
        assert p.procedure_id == "AI-SUPPLY.1"
    ...
    def test_maps_all_type_codes(self):
        w = _make_witness()
        for code_name, code_value in SUPPLY_RISK_CODES.items():
            p = w.witness_supply_chain_risk(10, 8, code_name)
            assert p.factor_c == float(code_value)

    def test_unknown_type_defaults(self):
        w = _make_witness()
        p = w.witness_supply_chain_risk(10, 8, "unknown_xyz")
    ...
    def test_context_at_clearing_level_1(self):
        ...
        assert "risk_level" in p.ai_context
