"""Tests for AI-DUALUSE.1 dual-use reporting witnessing."""
from unittest.mock import MagicMock
from swt3_ai.witness import Witness, REPORTING_STATUS_CODES
...
class TestWitnessDualUse:
    def test_mints_correct_procedure_and_factors(self):
        w = _make_witness()
        p = w.witness_dual_use(1, "notified", 30)
        assert p.procedure_id == "AI-DUALUSE.1"
    ...
    def test_maps_all_type_codes(self):
        w = _make_witness()
        for code_name, code_value in REPORTING_STATUS_CODES.items():
            p = w.witness_dual_use(1, code_name, 30)
            assert p.factor_b == float(code_value)

    def test_unknown_type_defaults(self):
        w = _make_witness()
        p = w.witness_dual_use(1, "unknown_xyz", 30)
    ...
    def test_context_at_clearing_level_1(self):
        ...
        assert "reporting_status" in p.ai_context
